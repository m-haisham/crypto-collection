...
def calculate_redundant_bits(bit_count):
...
def generate_parity_positions(length):
    index = 0
    while True:
        current = 2 ** index
        index += 1
...
def detect_error(data, is_even=True) -> Tuple[str, int]:
    if type(data) == str:
        data = [int(bit) for bit in data]

    length = len(data)
    binary = 0
    for i, p in enumerate(generate_parity_positions(length)):
